...
line-height:1.4;
';
hud.innerHTML='<div style="background:linear-gradient(135deg,#1e3a8a,#1e1b4b);
padding:12px 14px;
...
font-size:12.5px;
color:#fff;
">FB REAL MONETIZATION AUDITOR</div><div style="font-size:10px;
color:#93c5fd;
">Zero Guesswork • Live Screen&Tool Auditor</div></div></div><button id="raj-close-hud" style="background:rgba(255,255,255,0.1);
border:none;
color:#fff;
font-size:13px;
width:24px;
...
border-radius:50%;
cursor:pointer;
">✕</button></div><div style="padding:12px;
overflow-y:auto;
display:flex;
flex-direction:column;
gap:10px;
...
align-items:center;
justify-content:space-between;
"><span>1. Current Active Screen</span><button id="raj-btn-audit-screen" style="background:#2563eb;
color:#fff;
border:none;
padding:3px 8px;
border-radius:5px;
...
font-size:11px;
cursor:pointer;
">🔍 Audit Screen Now</button></div><div style="margin-bottom:6px;
"><label style="font-size:10.5px;
color:#94a3b8;
display:block;
margin-bottom:3px;
">Select Page Being Audited:</label><select id="raj-page-select" style="width:100%;
background:#1e293b;
color:#f8fafc;
...
font-size:11px;
outline:none;
"></select></div><div id="raj-screen-status" style="font-size:11px;
color:#cbd5e1;
background:rgba(0,0,0,0.3);
padding:6px;
border-radius:6px;
border:1px dashed rgba(255,255,255,0.15);
">Ready. Click Audit Screen Now to inspect active page.</div></div><div style="background:rgba(255,255,255,0.04);
border:1px solid rgba(255,255,255,0.08);
border-radius:8px;
padding:10px;
"><div style="display:flex;
...
color:#38bdf8;
">2. Fleet Pages(1-Click Open&Check)</span><select id="raj-fleet-select" style="background:#1e293b;
color:#93c5fd;
border:1px solid #475569;
border-radius:5px;
padding:3px 6px;
font-size:10px;
outline:none;
"></select></div><div id="raj-fleet-pages-list" style="max-height:120px;
overflow-y:auto;
background:#050914;
border:1px solid rgba(255,255,255,0.08);
border-radius:6px;
...
gap:3px;
font-size:11px;
"></div></div><button id="raj-btn-sync" style="background:linear-gradient(135deg,#059669,#10b981);
color:#fff;
border:none;
padding:10px;
border-radius:8px;
...
"><span>🚀 Send Verified Data to Dashboard</span><span id="raj-badge-count" style="background:#fff;
color:#052e16;
font-size:10.5px;
font-weight:900;
padding:2px 7px;
border-radius:10px;
">1 Ready</span></button></div>';
document.body.appendChild(hud);
//Populate fleet selector var fleetSel=document.getElementById('raj-fleet-select');
...
//Populate page selector&pages list function renderFleetUI(){
...
itemDiv.style.cssText='display:flex;
align-items:center;
justify-content:space-between;
padding:3px 4px;
border-bottom:1px solid rgba(255,255,255,0.04);
';
itemDiv.innerHTML='<span style="color:#fff;
overflow:hidden;
text-overflow:ellipsis;
white-space:nowrap;
max-width:180px;
">'+name+'</span><div style="display:flex;
gap:4px;
align-items:center;
">'+(isReady?'<span style="background:rgba(34,197,94,0.2);
color:#4ade80;
font-size:9.5px;
font-weight:800;
padding:1px 5px;
border-radius:4px;
">🟢 SET UP ACTIVE</span>':'<span style="color:#94a3b8;
font-size:9.5px;
">⚪ In Progress</span>')+'<a href="/'+pid+'/professional_dashboard/monetization/" style="color:#38bdf8;
...
if(verified[id]&&verified[id].status==='ready')readyCount++;
}
var badge=document.getElementById('raj-badge-count');
if(badge)badge.innerText=readyCount+' Ready';
}
updateCount();
document.getElementById('raj-close-hud').onclick=function(){
hud.remove();
}
;
//Screen Audit Logic function auditCurrent(){
...
//Find page name var pName='Selected Page';
for(var f in FLEETS){
for(var i=0;
i<FLEETS[f].length;
i++){
if(FLEETS[f][i][0]===selectedPid){
pName=FLEETS[f][i][1];
break;
}
}
...
font-weight:800;
">🟢 AVAILABLE TO SET UP:CONTENT MONETIZATION DETECTED!</span><div style="margin-top:2px;
color:#fff;
">Page:<strong>'+pName+'</strong>('+selectedPid+')</div><div style="font-size:9.5px;
color:#86efac;
margin-top:2px;
">[Set up] button verified live on screen!</div>';
verified[selectedPid]={
name:pName,status:'ready',tool:'Content Monetization',verifiedAt:new Date().toISOString()}
;
localStorage.setItem('raj_real_fb_audit',JSON.stringify(verified));
updateCount();
renderFleetUI();
...
el.innerHTML='<span style="color:#facc15;
font-weight:700;
">⏳ On Monetization Tab — Not Yet Eligible for Setup</span><div style="color:#cbd5e1;
">Page:'+pName+'</div>';
if(!verified[selectedPid]||verified[selectedPid].status!=='ready'){
verified[selectedPid]={
name:pName,status:'in_progress',tool:'None',verifiedAt:new Date().toISOString()}
;
localStorage.setItem('raj_real_fb_audit',JSON.stringify(verified));
}
renderFleetUI();
...
el.innerHTML='<span style="color:#94a3b8;
">Not on Monetization tab. Open<strong>Professional Dashboard&gt;
Monetization</strong>for this page,or click Open ↗ in the list below.</span>';
}
}
