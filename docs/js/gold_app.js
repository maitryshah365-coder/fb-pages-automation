...
/* ==========================================================================
   CYBER MATRIX RAIN & STEALTH HACKER CANVAS ENGINE
   Ultra-smooth, battery-efficient, adaptive 60FPS for Desktop & Mobile Phone
   ========================================================================== */
function initCyberMatrixEngine() {
  ...
  // Authentic Matrix Katakana + Hex Byte Strings + Binary + Operator Glyphs
  const katakana = 'ｦｱｳｴｵｶｷｹｺｻｼｽｾｿﾀﾂﾃﾅﾆﾇﾈﾊﾋﾎﾏﾐﾑﾒﾓﾔﾕﾗﾘﾜ0123456789ABCDEF';
  const cyberChars = '01010101<>{}/*#$_+~=!%&';
  const alphabet = katakana + cyberChars;
  ...
  function resetMatrixGrid() {
    ...
    for (let i = 0; i < columns; i++) {
      // Stagger drops randomly so streams fall naturally across screen
  ...
  let lastFrameTime = 0;
  // 30 FPS on mobile (cool battery, smooth rain), 36 FPS on desktop
  const targetFPS = isMobile ? 30 : 36;
  const frameInterval = 1000 / targetFPS;

  function renderMatrixStream(timestamp) {
    ...
    // Subtle dark translucent sweep creating phosphor decay trails
    ctx.fillStyle = 'rgba(2, 6, 11, 0.088)';
    ...
    for (let i = 0; i < drops.length; i++) {
      ...
      if (y > 0 && y < height + fontSize * 2) {
        // Glowing cyan-white leading streamer head
        ctx.fillStyle = '#e0fff4';
        ctx.shadowColor = '#00ff66';
        ctx.shadowBlur = 8;
        ctx.fillText(char, x, y);

        // Trailing phosphor matrix green character
        ctx.fillStyle = '#00ff66';
        ctx.shadowBlur = 4;
        const prevChar = alphabet.charAt(Math.floor(Math.random() * alphabet.length));
      ...
      // Reset stream to top when falling past bottom with organic variance
  ...
  window.addEventListener('resize', () => {
    ...
    resizeTimer = setTimeout(resetMatrixGrid, 200);
  }, { passive: true });
}

window.initCyberMatrixEngine = initCyberMatrixEngine;

// Launch automatically when DOM is ready
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initCyberMatrixEngine);
} else {
  initCyberMatrixEngine();
}
